...
export function convertWebToMobile(webUrl) {
  ...
  // Finde den Basis-Typ, indem wir prüfen, ob der URL-Teil mit einem bekannten Schlüssel *beginnt*.
  // z.B. findet er 'wohnung-kaufen' in 'wohnung-kaufen-mit-balkon'
  const baseTypeKey = Object.keys(REAL_ESTATE_TYPE).find(key => realTypeKey.startsWith(key));

  if (baseTypeKey) {
    realType = REAL_ESTATE_TYPE[baseTypeKey];
  }
