...
async function getListings(url) {
  ...
  // 4. Fetch details with concurrency limit
  const CONCURRENCY_LIMIT = 5; // Adjust as needed
  const results = [];
  const listingsToFetch = allListings.filter((item) => item.type === 'EXPOSE_RESULT');

  // Helper to process a single item
  const processItem = async (expose) => {
    ...
    try {
      ...
      if (exposeResponse.ok) {
        ...
        logger.warn(`Failed to fetch expose for ${item.id}: ${exposeResponse.status} ${exposeResponse.statusText}`);
      }
    } catch (e) {
  ...
  for (let i = 0; i < listingsToFetch.length; i += CONCURRENCY_LIMIT) {
    const chunk = listingsToFetch.slice(i, i + CONCURRENCY_LIMIT);
