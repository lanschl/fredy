...
function extractNumber(text) {
  if (!text) return null;
  // Entfernt alles außer Zahlen und dem Komma als Dezimaltrennzeichen
...
async function getListings(url) {
  ...
  const listingPromises = responseBody.resultListItems
    ...
    .map(async (expose) => {
      ...
      const numericPrice = extractNumber(price?.value);
...
function normalize(o) {
  ...
  const address = nullOrEmpty(o.address_full) ? 'NO ADDRESS FOUND' : (o.address_full || '').replace(/\(.*\),.*$/, '').trim();
