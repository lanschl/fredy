...
class FredyPipeline {
   ...
   */
  _filter(listings) {
    // If crawlFields is defined (old provider style), validate that all fields are present.
    // If not defined (new provider style with getListings), skip this check.
    let validatedListings = listings;
    if (this._providerConfig.crawlFields) {
      const keys = Object.keys(this._providerConfig.crawlFields);
      validatedListings = listings.filter((item) => item && keys.every((key) => key in item));
    }
    return validatedListings.filter(this._providerConfig.filter);
  }
